...
export async function approvalRequests(companyId: string) {
  const [pending, resolved] = await Promise.all([
    db.approvalRequest.findMany({ where: { companyId, status: 'PENDING', kind: 'STATUS_CHANGE' }, orderBy: { createdAt: 'asc' }, take: 200 }),
    db.approvalRequest.findMany({ where: { companyId, status: { not: 'PENDING' }, kind: 'STATUS_CHANGE' }, orderBy: { resolvedAt: 'desc' }, take: 30 }),
  ]);
  const payloadOf = (p: unknown) => p as StatusChangePayload;
  const itemIds = [...new Set([...pending, ...resolved].flatMap((r) => payloadOf(r.payload).itemIds ?? []))];
  const [items, statuses] = await Promise.all([
    itemIds.length
      ? db.item.findMany({
          where: { companyId, id: { in: itemIds } },
          select: { id: true, serial: true, dupNote: true, contractItem: { select: { contractId: true } }, status: { select: { name: true, color: true } } },
        })
      : [],
  ...
  const shape = (r: (typeof pending)[number]) => {
    ...
    return {
      ...
      items: (p.itemIds ?? []).map((id) => byId.get(id)).filter((x): x is NonNullable<typeof x> => !!x),
      missing: (p.itemIds ?? []).filter((id) => !byId.has(id)).length,
    };
  };
