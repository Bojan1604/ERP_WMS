...
export function Combobox({
  ...
  // otvaranje ističe trenutni odabir (ili „— bez odabira —"), ne prvu stavku; tipkanje/strelice/miš preuzimaju isticanje
  const [touched, setTouched] = useState(false);
  useEffect(() => {
    if (st.open) setTouched(false);
  }, [st.open]);
  const valueIndex = value ? list.findIndex((o) => o.value === value) : -1;
  useEffect(() => {
    if (st.open && !touched && !st.q && valueIndex > 0 && st.active !== valueIndex) dispatch({ type: 'hover', index: valueIndex });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [st.open, touched, st.q, valueIndex]);
  const emptyActive = !!allowEmpty && !value && !touched && !st.q;
  ...
  const onInputKey = (e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
      e.preventDefault();
      // prva strelica dolje s „— bez odabira —" ide na prvu stavku
      if (emptyActive && e.key === 'ArrowDown') setTouched(true);
      else {
        setTouched(true);
      ...
      e.preventDefault();
      // istaknut je „— bez odabira —": Enter ga potvrđuje
      if (emptyActive) dispatch({ type: 'pick', option: null });
      else dispatch({ type: 'enter', list });
      ...
      <Popover anchor={btn} open={st.open} panelRef={panel}>
        <div className="relative shrink-0">
          <input
            ...
            aria-activedescendant={emptyActive ? `${uid}-none` : list.length ? optId(st.active) : undefined}
        ...
        <ul id={listId} role="listbox" className={cn('min-h-0 flex-1 overflow-y-auto scroll-slim py-1', st.loading && st.remote && 'opacity-60')}>
          {allowEmpty && (
            <li
              ...
              className={cn('cursor-pointer px-3 py-1.5 text-fg-3 hover:bg-muted max-sm:py-2.5', emptyActive && 'bg-muted')}
            >
              — bez odabira —
            </li>
          )}
          {list.map((o, i) => (
            <li
              ...
              className={cn('flex cursor-pointer items-center gap-2 px-3 py-1.5 max-sm:py-2.5', i === st.active && !emptyActive && 'bg-muted')}
            >
