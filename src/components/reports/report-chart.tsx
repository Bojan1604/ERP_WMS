...
export function ReportChart({ spec, title }: { spec: Spec; title: string }) {
  ...
  return (
    <Card title={title} className="mb-4">
      <BarChart data={spec.data} series={spec.series} stacked={spec.stacked ?? true} format={fmt(spec.unit)} height={spec.data.length > 8 ? 260 : 220} width={1100} ariaLabel={title} />
    </Card>
  );
}
